...
class Mp4 {
public:
    int render(int argc, char **argv,uint32_t w, uint32_t h)
    {
		fileName = argv[1];
        auto player = rlottie::Animation::loadFromFile(fileName);
        if (!player) return 0;
        ...
        VideoCapture video;
        video.Init(argc, argv, w, h, rate, 1000);
		std::cout << frameCount << std::endl;
        for (size_t i = 0; i < frameCount; i++) {
            rlottie::Surface surface(buffer.get(), w, h, w * 4);
            player->renderSync(i, surface);
			argbTorgba(surface);
            video.AddFrame(reinterpret_cast<uint8_t *>(surface.buffer()));
			std::cout << i << std::endl;
        }
        video.Finish();
    }
